...
xls_path="${XL_IDP_PATH_REFERENCE}/reference_is_empty/"
...
do
  ...
  if [ $? -eq 0 ]
	...
	python3 ${XL_IDP_PATH_REFERENCE_SCRIPTS}/scripts_for_bash_with_inheritance/convert_csv_to_json.py "${csv_name}" "${xls_path}"/json/$(basename "${csv_name}")
